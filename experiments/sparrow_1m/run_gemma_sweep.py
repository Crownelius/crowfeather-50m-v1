"""Run head-to-head Sparrow vs google/gemma-3-27b-it across all canonical iters.

Identical structure to run_owl_sweep.py but points at Gemma instead and writes
to a `_gemma.json` suffix so the Owl scoreboard isn't overwritten.
"""
import json
import os
import subprocess
...
LADDER = [
    ...
    ("iter15",  "factor",     1),
    ("iter16",  "diff",        1),
    ("iter17",  "collatz_step", 3),
    ("iter18",  "collatz_time", 3),
    ("iter19",  "goldbach",     3),
    ("iter20",  "fermat_little", 2),
]

KEY_PATH = r"D:\FANT_TRAINING_D_Drive\fant2\.openrouter_runtime_key"
ARTIFACT_ROOT = "E:/sparrow"
N_PER_TASK = 50
# Gemma 3 27B paid endpoint — much higher rate limit than Owl Alpha free tier;
# ~1 req/sec works fine. Cost ~$0.10/M tokens; full 20-iter sweep ~250K tokens
# = ~$0.03 from the user's $14.85 OpenRouter balance.
RPS = 1.0
SCORING = "numeric"
BASELINE = "google/gemma-3-27b-it"
REPORT_SUFFIX = "_gemma"
...
def main():
    ...
    for idx, (iter_dir, task, digits) in enumerate(LADDER):
        ...
        report_path = f"{sparrow_path}/eval_{task}_{digits}d{REPORT_SUFFIX}.json"
        ...
        try:
            ...
            print(f"    DONE in {elapsed:.1f}s   "
                  f"Sparrow={sp_acc*100:.1f}%  Gemma={bl_acc*100:.1f}%  "
                  f"delta={delta*100:+.1f}pp  failed={n_failed}")
            results.append({
                "iter": iter_dir, "task": task, "digits": digits,
                "status": "ok", "elapsed_sec": elapsed,
                "sparrow_acc": sp_acc, "gemma_acc": bl_acc, "delta_pp": delta * 100,
                "gemma_failed": n_failed,
                "sparrow_p50_ms": report["sparrow"]["p50_latency_ms"],
                "gemma_p50_ms": bl["p50_latency_ms"],
                "gemma_usage": bl.get("usage", {}),
            })
    ...
    for r in results:
        ...
        print(f"{r['iter']:10s} {r['task']:12s} {r['digits']:>6d}  "
              f"{r['sparrow_acc']*100:>7.1f}% {r['gemma_acc']*100:>7.1f}%  "
    ...
    summary_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "gemma_sweep_summary.json")
